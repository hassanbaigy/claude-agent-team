...
export async function runInit(opts) {
  ...
  if (!preset) {
    throw new Error(`unknown preset "${presetName}". Use minimal | standard | full.`);
  }
  ...
  log(`${C.dim}preset: ${presetName} (${preset.length} agents) — stack: ${stack.summary}${C.reset}${dryRun ? `  ${C.yellow}[dry-run]${C.reset}` : ""}`);
  log("");
